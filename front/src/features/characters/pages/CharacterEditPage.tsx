...
type CharacterTab = 'characteristics' | 'inventory' | 'grimoire' | 'features'
...
export function CharacterEditPage() {
  ...
  const tabsOrder: CharacterTab[] = ['characteristics', 'inventory', 'grimoire', 'features']

  useEffect(() => {
    async function loadName() {
      if (!id) return
      try {
        const res = await apiGet<{ success: boolean; character: { name: string } }>(`/api/characters/${id}`, token)
  ...
  return (
    <Card title={`Edition du personnage ${characterName.trim() || (id ? `#${id}` : '')}`}>
